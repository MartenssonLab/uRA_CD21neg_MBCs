...
DimPlot(uRA4.1, label = TRUE, pt.size = 2, label.size = 7, 
...
## C3 Subclusters
uRA4.3 = readRDS("uRA4_C3_subs")

tree = BuildClusterTree(uRA4.3, assay = "SCT")
...
## All clusters
tree = BuildClusterTree(uRA4.1, assay = "SCT")
myphytree = Tool(tree, slot = "BuildClusterTree")
ggtree(myphytree) + 
	geom_tiplab() + 
	theme_tree() + 
...
top10_up_degs = read.xlsx("reports/uRA4_C3_subs.xlsx", 
	sheetName = "Top10_Up", col.names = TRUE, row.names = FALSE)
top10_down_degs = read.xlsx("reports/uRA4_C3_subs.xlsx", 
	sheetName = "Top10_Down", col.names = TRUE, row.names = FALSE)

DoHeatmap(uRA4.3, features = top10_up_degs$gene, raster = FALSE, 
	group.colors = c("#00C08B", "#00B4F0", "#C77CFF")) # Plot top10 up DEGs

DoHeatmap(uRA4.3, features = top10_down_degs$gene, raster = FALSE, 
...
DoHeatmap(uRA4.3, features = genes, raster = FALSE, 
...
p = round(table(Idents(uRA4.1)) / sum(table(Idents(uRA4.1))
